...
- **Estado Actual**: Fases 81-86 completadas y mergeadas a desarrollo. Fase 87 (Modal Responsive Fix) pendiente de commit. 2 cursos visibles en DB Free (Psicología + Ingeniería Mecatrónica). 219 registros en `cleansed_programs` pendientes de enrichment. RLS ya filtra `is_active=true AND is_verified=true` para anon. `COURSE_PUBLIC_FIELDS` ya excluye columnas internas.
- **Último Hito**: Fase 86 — Quick Compare desde Catálogo (checkbox en tarjetas, máx 3, localStorage sync con detail page). Fase 85 — Deployment Fix + UI Polish. Fase 79A (encoding) resuelto — archivos frontend están en UTF-8 puro, sin mojibake.
- **Próxima Acción**: Commit + push Fase 87 (Modal Responsive Fix) a desarrollo. Luego ejecutar pipeline FG2 (enrichment_worker → sync_vector_worker) para poblar DB con cursos reales.
...
|---|---|---|---|---|
| **P0** | **Fase 76 — Hotfix Pipeline FG2 (bugs restantes)** | **Pipeline** | **Bugs pendientes de los 7 originales:** (1) `stealth_async` undefined en línea 402 de `universal_harvester.py` — usa función no importada en vez de `Stealth.apply_stealth_async()`, (5) JSONB string-vs-array validación pendiente (Fase 79D). Bugs ya corregidos: (1) `self.discovery_mode` ✅, (2) `DNSResolutionError` fallback ✅, (6) `error_message`→`processing_error` en cleansing ✅, (7) `error_message` en enrichment ✅. | **Ninguno — branch hotfix** |
...
| **P2** | **Fase 87 — Modal Solicitar Asesoría: Responsive Fix** | Frontend + UX | En pantallas pequeñas (≤667px), el botón X del modal "Solicitar Asesoría" se perdía fuera del viewport. Fix: overlay con `items-start + pt-4 + overflow-y-auto`, header sticky con X siempre visible, contenido scrolleable con `max-h-[70vh]`, click en backdrop cierra el modal. | Ninguno |
...
- [ ] **Fase 87**: Modal Solicitar Asesoría Responsive Fix — Botón X siempre visible en móvil, header sticky, scroll interno, backdrop click cierra modal.
...
| categories | 17 | 17 | ✅ 17/17 mapeados por slug |
...
| **courses** | **12** | **12** (UTP) | ✅ Synced (slug mapping) |
| **staging_raw** | **6,498** | **6,498** | ✅ Synced (UPSERT por URL) |
| cleansed_programs | 242 | 242 | ✅ Synced |
| enriched_programs | 12 | 12 | ✅ Synced (INSERT directo) |
| leads | 0 | 0 | N/A |
...
1. **Crear script de sincronización Pro→Free**:
    - [x] Crear `scripts/maintenance/sync_pro_to_free.py` con slug mapping automático
    ...
    - [x] Mapeo automático: construir diccionario `pro_uuid → slug → free_uuid` para `institution_id` y `category_id`
    - [x] Modo `--dry-run`: contar filas y mostrar mapping sin escribir
    - [x] Modo `--full`: sincronizar las 4 tablas (default)
    - [x] Modo `--table <tabla>`: sincronizar solo una tabla específica
...
2. **Probar sincronización con dry-run**:
    ...
    - [x] Verificar que no hay UUIDs huérfanos (institution_id/category_id que no existen en Free)
...
3. **Ejecutar sincronización real**:
    ...
    - [x] Verificar en Free Dashboard: `SELECT count(*) FROM staging_raw` retorna ≥6,498
...
- **Causa**: En `__init__` (líneas 59-70) se asignan `self.site_type`, `self.requires_stealth`, etc. desde el perfil, pero **nunca se asigna `self.discovery_mode`**. Sin embargo, `main()` (línea 715) accede a `harvester.discovery_mode`.
...
- **Causa**: El bloque `except` en `_request_with_retry()` catch `requests.exceptions.DNSResolutionError`, pero esta excepción **no existe** en la versión instalada de `requests`. Se introdujo en `requests>=2.32` (2024) pero el workflow instala la versión disponible. Resultado: cualquier error de DNS/connection lanza `AttributeError` en vez de ser manejado → crash silencioso sin retry.
...
- **Causa**: Construye URLs manualmente con `requests.get(url, headers=headers)` en vez de usar `db.select_all()`. No tiene retry logic, no usa service_role key, y falla si `SUPABASE_URL` no está configurado sin el prefijo `https://`.
...
- **Causa**: Las variables de entorno tienen `\$` en vez de `$`, por lo que GitHub Actions no resuelve los secrets. Resultado: `SUPABASE_URL` queda vacío → `Invalid URL '/rest/v1/...'`.
...
- **Causa**: PUCP tiene 3 campos JSONB almacenados como `string` en vez de `array`. Cuando el harvester itera `for pattern in catalog_url_patterns:` sobre un string, itera carácter por carácter → `page.goto("[" )`, `page.goto("h")`, `page.goto("t")`, etc. UTP tiene 2 campos afectados (`allowed_url_patterns`, `exclusion_patterns`). PostgREST retorna strings JSON como listas de caracteres individuales.
- **Impacto**: FG2 en producción (run `25300423582`) generó 884 navegaciones inválidas a URLs de 1 carácter (`[`, `h`, `t`, `t`, `p`, `:`, `/`, etc.) que saturaron Playwright y cancelaron el pipeline por timeout.
- **Datos corruptos (Free DB)**:
  - `pucp`: `catalog_url_patterns` = `'["https://www.pucp.edu.pe/programas/?jsf=jet-engine&pagenum={page}"]'` (68 chars como string) → debe ser `["https://..."]` (1-element array)
  - `pucp`: `allowed_url_patterns` = `'["re:.*programas.*"]'` (string) → debe ser `["re:.*programas.*"]`
  - `pucp`: `exclusion_patterns` = `'["/blog/", "/noticias/", ...]'` (string) → debe ser `["/blog/", ...]`
  - `utp`: `allowed_url_patterns` = `'["re:/cgt/"]'` (string) → debe ser `["re:/cgt/"]`
  - `utp`: `exclusion_patterns` = `'["/categoria/", ...]'` (string) → debe ser `["/categoria/", ...]`
- **Pro DB**: Faltan perfiles para PUCP y DMC (solo 10 de 12 instituciones tienen `institution_site_profiles`).
- **Fix SQL (Free)**:
  ```sql
  UPDATE institution_site_profiles
  SET catalog_url_patterns = catalog_url_patterns::jsonb
  WHERE jsonb_typeof(catalog_url_patterns) = 'string';

  UPDATE institution_site_profiles
  SET allowed_url_patterns = allowed_url_patterns::jsonb
  WHERE jsonb_typeof(allowed_url_patterns) = 'string';

  UPDATE institution_site_profiles
  SET exclusion_patterns = exclusion_patterns::jsonb
  WHERE jsonb_typeof(exclusion_patterns) = 'string';
  ```
...
| Archivo | Tipo | Bug |
|---|---|---|
| `scripts/core/universal_harvester.py` | FIX | Bug 1: agregar `self.discovery_mode` |
| `scripts/shared/db_client.py` | FIX | Bug 2: `getattr()` retrocompatible |
