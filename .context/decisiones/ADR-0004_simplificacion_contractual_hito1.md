...
6. La decision humana posterior a PR #262 reconcilia F9.7 con un PR-O sucesor aun no implementado mediante executor privado digest-bound, target-bound y single-use, no expuesto por Data API. Esto reemplaza cualquier ruta basada en `public.exec_sql(text)` como estado final esperado y no concede `GO_FOR_FREE`.
