...
La evidencia no inventa campos no observables. El historial REST de approvals se procesa desde `environments[]`, `user`, `state`, `comment` y `created_at`. `approval_id` dentro del envelope es solo referencia humana/JIT; la identidad derivada del review observado es `environment_review_digest=sha256(canonical_json(environment_review))`. El comentario libre no se persiste: se conserva solo `comment_present` y, si aplica, `comment_sha256`.
