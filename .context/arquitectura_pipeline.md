...
Snapshot GOV-CI12: `desarrollo@793a2fb5aabc9e23bba2e3d36b47d6826444c5d4`, tree `bb3a9084961c090adac0d390aa22fdcc84670656`; PR #444 publico CI11, PR #445 queda `FAILED_NOT_MERGED_FROZEN`, HOM-011 O2 consumido y HOM-011 O3-O5 superseded. GOV-CI12 reemplaza la familia runtime por HOM-012 con collector Python testeable, envelope v3, evidencia pre-merge inmutable sanitizada, post-merge sin secret y cierre O3 asincronico consumible por O4. REQ6 elimina del job `db-sync-detect-only` el checkout dinamico y la ejecucion de codigo del candidate para corregir CodeQL `security/code-scanning/19` en PR #446.
...
- GOV-CI12: promociones futuras usan HOM-012 exacto, `promotion-jit-envelope-v3`, una sola aprobacion Environment `Promotion` pre-merge, `promotion-approval-evidence.json` inmutable y post-merge sin secret. `bypass_actors` ausente es `UNOBSERVABLE`; O3 produce `o3-closure-evidence.json`; O4 lo consume por loader real y O5 valida trees/ancestry finales. La ruta O3 no-change de `db-sync-detect-only` genera su artifact desde workflow trusted inline sin checkout ni scripts del candidate.
