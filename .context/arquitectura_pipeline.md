...
Snapshot GOV-CI11: `desarrollo@cbdfe9dab373a2b427df4864b14427f3b2358789`, tree `99c1cda4f0091aaee35752caec69745051c41a3a`; PR #443 queda `FAILED_NOT_MERGED_FROZEN`, HOM-010 O2 consumido y HOM-010 O3-O5 superseded. GOV-CI11 reemplaza la familia runtime por HOM-011 con envelope transaccional, readiness read-only, cierre O3 Cloudflare/DB detect-only y bloqueo de O4 hasta cierre O3.
Snapshot GOV-CI12 REQ7: `desarrollo@34fb06d552ea4d052bd3f942948a192a7e44a5eb`, tree `d53e81717a4c44d59177591c3ce69392072ec716`; PR #446 y PR #448 fueron publicados en `desarrollo`; PR #443, PR #445 y PR #447 quedan `FAILED_NOT_MERGED_FROZEN`. HOM-012 REQ1 O2 queda consumido y O3-O5 REQ1 quedan superseded. GOV-CI12 REQ7 prepara ramas/grants REQ2 para HOM-012 con collector Python testeable, envelope v3, evidencia pre-merge inmutable sanitizada, post-merge sin secret y cierre O3 asincronico consumible por O4.
...
- GOV-CI11: promociones futuras usan HOM-011 exacto, `R3_JIT_APPROVAL_ENVELOPE` schema `promotion-jit-envelope-v1`, `opened` attempt 1, sin cancelacion del run valido, eventos mutantes/rerun invalidos, `Promotion.can_admins_bypass=false`, O3 con Cloudflare Pages app_id `85455` y `DB Sync Detect Only=NO_DB_CHANGES`, O4 bloqueado hasta cierre O3.
- GOV-CI12: promociones futuras usan HOM-012 REQ2 exacto, `promotion-jit-envelope-v3`, una sola aprobacion Environment `Promotion` pre-merge, `promotion-approval-evidence.json` inmutable y post-merge sin secret. `bypass_actors` ausente es `UNOBSERVABLE` y no equivale a bypass vacio; cuando es observable debe validar bypass exclusivo de `romelhc95`. O3 produce `o3-closure-evidence.json`; O4 lo consume por loader real y O5 valida trees/ancestry finales. La ruta O3 no-change de `db-sync-detect-only` genera su artifact desde workflow trusted inline sin checkout ni scripts del candidate.
