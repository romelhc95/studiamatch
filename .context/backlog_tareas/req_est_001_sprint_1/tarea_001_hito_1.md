...
Decisiones vinculantes: todos los roles de aplicacion, incluido `service_role`, quedan sin acceso a `leads`/`email_log` en el package local; la publishable key historica retirada tiene estado `ROTATED_HUMAN_ATTESTED`, sin registrar su valor. El siguiente paso operativo pertenece a gates separados de Free para llegar a `GO_F9.7_COMPLETE`; esta reconciliacion no autoriza Supabase Free/Pro, DDL/DML, backup, writers, backfill, deploy ni Cloudflare manual.
