...
Estado del corte local: `public_lead_capture=LOCAL_CODE_REMOVED_REMOTE_UNKNOWN`, `email_egress=LOCAL_TOMBSTONE_REMOTE_UNKNOWN`, `security_hold=LOCAL_CANDIDATE_BLOCKED`, `WP-F9.7-02=GO_WP_LOCAL`, `WP-F9.7-03=GO_WP_LOCAL`, `WP-F9.7-04=GO_WP_LOCAL`, T02 `NOT_EXECUTED`, backup `PLANNED`, writers `INVENTORIED`, Free/Pro `UNCHANGED_NOT_ATTESTED`, siguiente accion `execute_wp_f9_7_05_freeze_candidate`. La captura publica no puede reactivarse por configuracion; cualquier reactivacion vive en [BK-F9.5-05](backlog_seguridad_leads_email.md).
...
| `WP-F9.7-04` | `COMPLETED` | CI, hooks, EOL, actionlint y protected gates en `GO_WP` |
| `WP-F9.7-05` | `READY_FOR_VALIDATION` | Candidate inmutable y matriz acumulada en `GO_WP` |
| `WP-F9.7-06` | `BLOCKED_BY_WP05` | Seis auditorias `GO_FOR_LOCAL_PR`, push y PR sin merge |

Decisiones vinculantes: todos los roles de aplicacion, incluido `service_role`, quedan sin acceso a `leads`/`email_log`; la publishable key historica retirada tiene estado `ROTATED_HUMAN_ATTESTED`, sin registrar su valor. El siguiente paso es ejecutar exclusivamente `WP-F9.7-05`.
