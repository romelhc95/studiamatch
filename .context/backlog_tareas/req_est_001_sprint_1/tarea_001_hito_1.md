...
| `WP-F9.7-05` | `COMPLETED` | Candidate inmutable y matriz acumulada en `GO_WP` |
| `WP-F9.7-06` | `READY_FOR_AUDIT` | Seis auditorias `GO_FOR_LOCAL_PR`, push y PR sin merge |

Decisiones vinculantes: todos los roles de aplicacion, incluido `service_role`, quedan sin acceso a `leads`/`email_log`; la publishable key historica retirada tiene estado `ROTATED_HUMAN_ATTESTED`, sin registrar su valor. El siguiente paso es ejecutar exclusivamente `WP-F9.7-06`.
