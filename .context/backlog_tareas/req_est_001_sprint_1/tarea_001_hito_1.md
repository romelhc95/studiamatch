...
| Fase vigente | Macrofase `F9` completada y `F10.10` en `M3_READER_ROTATION_ATTESTED_PREFLIGHT_GATE_PENDING`. F10.9 conserva `G4=STOP_REQUIRES_REBASELINE`. PR #353 promovio M3 reader v2; PR #354 y CI post-merge validaron su reconciliacion. La [atestacion sanitizada de rotacion](../../operaciones/m3_reader_f10_10_rotation_attestation_2026_08_11.md) cierra el hallazgo de la contrasena SQL canary anterior. Ningun gate esta consumido; payload y aprobacion exacta de preflight siguen pendientes. Certification, Pro y M4-M10 no estan autorizados. Hito 1 permanece `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
4. `M3`: `M3_READER_ROTATION_ATTESTED_PREFLIGHT_GATE_PENDING`; PR #350 permanece antecedente v1. PR #353/#354 promovieron y reconciliaron el package v2 solo `FREE_DB`; CI post-merge valido boundary, aislamiento Pro y PostgreSQL 17 networkless. La rotacion SQL canary queda atestada; preflight, DDL Free, Q0, lectura y teardown no iniciaron y requieren gates exactos independientes.
...
F10.10 no habilita Certification, Pro, M4, F10.9/G4, schedules ni F11.1 y no copia datos entre ambientes. En este rebaseline no hubo red remota, Supabase, DDL/DML remoto, password ni consumo de `APPROVE_M3_FREE_READONLY`.
