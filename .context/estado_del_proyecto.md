...
| `F10.10` | `M3_READER_PREFLIGHT_PASS_DDL_GATE_PENDING` | [Scope M3 v2](operaciones/m3_f10_10_scope_por_ambiente_target.md), [payload](operaciones/m3_reader_f10_10_preflight_payload_2026_08_11.json) y [evidencia](operaciones/m3_reader_f10_10_preflight_evidence_2026_08_11.md) fijan `FREE_DB` como unico target. Preflight local passwordless PASS, `network=none`, sin password ni capacidad remota. `APPROVE_F10_10_M3_READER_PREFLIGHT_FREE` consumido una vez; DDL, Q0, lectura y teardown no consumidos. |
...
- Siguiente accion: promover y validar la proyeccion local fail-closed para `apply_migration`; despues preparar para revision humana un payload exacto de `APPROVE_F10_10_M3_READER_DDL_FREE` con candidate/tree post-merge, binding vigente y `applied_query_digest`. El gate preflight queda consumido; DDL Free, Q0 Free, lectura y teardown permanecen no consumidos y ninguno concede el siguiente.
