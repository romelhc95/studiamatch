...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.10 queda `M3_DDL_NULLABILITY_REMEDIATION_PENDING`: preflight PASS; DDL v1 consumida una vez y revertida por drift de nullability; Q0, lectura y teardown no consumidos. La remediacion v2 permanece offline, sin binding ni capacidad ejecutable. F10.9 conserva `STOP_REQUIRES_REBASELINE`; Hito 1 sigue `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
| `F10.10` | `M3_DDL_NULLABILITY_REMEDIATION_PENDING` | [Scope M3 v2](operaciones/m3_f10_10_scope_por_ambiente_target.md) y el [diagnostico offline](operaciones/m3_reader_f10_10_nullability_remediation_2026_08_12.json) registran `STOP_COURSES_COLUMN_CONTRACT_DRIFT`: la unica llamada v1 hizo rollback porque el package exigia `courses.is_active NOT NULL`, mientras DB-as-Code lo define nullable con `DEFAULT true`. La remediacion v2 queda sin binding y no ejecutable. DDL v1 fue consumido/fallido; Q0, lectura y teardown no consumidos. |
...
- Subfase tecnica activa: F10.10 `M3_DDL_NULLABILITY_REMEDIATION_PENDING`. F10.9 conserva `STOP_REQUIRES_REBASELINE`; G5-G13, Certification, Pro, M4-M10, schedules, observacion y F11.1 siguen suspendidos. DDL v1 fue consumida/fallida con rollback y no concede retry; la remediacion v2 no tiene binding ni gate ejecutable.
...
- Siguiente accion: revisar la remediacion offline de nullability y sus digests v2. La identidad v1 queda superseded y no reutilizable; no se genera binding ni payload DDL ejecutable hasta el merge y una nueva preparacion aprobada. Q0, lectura y teardown permanecen no consumidos.
