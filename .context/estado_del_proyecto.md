...
Snapshot: `SNAPSHOT-2026-08-05-F10.8-BLOCKED-ENVIRONMENT-VARIABLE`.
...
- Subfase tecnica activa: F10.8 `IN_PROGRESS_BLOCKED_ENVIRONMENT_VARIABLE`; [ADR-0008](decisiones/ADR-0008_rebaseline_f10_7_gate_reconstruction.md) documenta el rebaseline inicial y [ADR-0009](decisiones/ADR-0009_reconciliacion_entrega_tecnica_post_main_f10_7.md) registra la entrega tecnica post-main. F10.7 permanece `COMPLETED_TECHNICAL_DELIVERY` y conserva [PLAN-F9.7-CIERRE-001](operaciones/cierre_definitivo_f9_7.md), [PR-O v1 superseded](operaciones/pr_o_f9_7_v3_hold.md) y [PR-O executor privado](operaciones/pr_o_f9_7_successor_private_executor.md) como historia no ejecutable de Hito 1.
...
- Tarea pendiente de F10.8: corregir la configuracion del environment `Production` agregando la variable no secreta `F10_PRODUCTION_CANARY_SUPABASE_HOST` por valor privado fuera de Git y reintentar una unica vez el canary Production manual con `candidate_sha=64e4ed895d43121c5683e26a355993f18e528a5c`, tree `7d43590c19ca15171d468bf8c823a5e93b47d8cc`, workflow blob `8959c4b8f90afe0d75f23bb6e0f99d51321f1c45`, cohorte `institution_slug` seleccionada privadamente, `USER_PERSONAL_UAT=PASS` nuevo y aprobacion separada del environment `Production`. No hay autorizacion de Supabase Free/Pro fuera del workflow aprobado, DDL/DML, backup/restore real fuera del snapshot privado del canary, writers remotos no acotados, backfill, Edge, schedules ni CA2.
...
- Siguiente accion futura: revisar y fusionar esta formalizacion documental a `desarrollo`; despues, solicitar la frase exacta `Ejecuta las tareas pendientes de la Fase F10.8` antes de configurar `F10_PRODUCTION_CANARY_SUPABASE_HOST` o crear otro workflow dispatch. Schedules siguen bloqueados hasta F10.9.
