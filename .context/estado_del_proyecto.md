...
Snapshot: `SNAPSHOT-2026-08-10-F10.10-M3-SCOPE`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.8 queda `COMPLETED_PRODUCTION_CANARY_VERIFIED`; F10.9 conserva `STOP_REQUIRES_REBASELINE`. [M2 post-merge](operaciones/m2_f10_10_post_merge_evidence_2026_08_10.md) registra M1/M2 PASS. El [scope M3](operaciones/m3_f10_10_scope_por_ambiente_target.md) queda aprobado, con ejecucion bloqueada hasta gates separados Free/Certification/Pro. Hito 1 sigue `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
| `F10.10` | `M3_COLLECTOR_CANDIDATE_PENDING_PROTECTED_MERGE` | [ADR-0010](decisiones/ADR-0010_rebaseline_f10_10_metadata_remediation.md), [PLAN-F10.10-001](operaciones/plan_remediacion_metadata_f10_10.md), [evidencia M2](operaciones/m2_f10_10_post_merge_evidence_2026_08_10.md) y [scope M3](operaciones/m3_f10_10_scope_por_ambiente_target.md) preservan la frontera fill-only/sin DDL. El collector candidate y zero-write tests requieren merge protegido; ninguna lectura remota esta autorizada. |
...
- Subfase tecnica activa: F10.10 `M3_COLLECTOR_CANDIDATE_PENDING_PROTECTED_MERGE`. F10.9 conserva `STOP_REQUIRES_REBASELINE`; G5-G13, schedules y observacion siguen suspendidos. El candidate no concede llamadas remotas: Free, replay Certification y Pro conservan gates separados.
...
- Siguiente accion: revisar y promover por PR protegido el collector candidate `scripts/maintenance/f10_10_m3_readonly_collector.py` y sus zero-write tests. Solo tras evidencia post-merge puede aprobarse `APPROVE_M3_FREE_READONLY` con el payload completo. MCP solo puede ser sanity-check no autoritativo; Certification y Pro permanecen bloqueados.
