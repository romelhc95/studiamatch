...
Snapshot: `SNAPSHOT-2026-08-11-F10.10-M3-READER-REBASELINE-CANDIDATE`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.8 queda `COMPLETED_PRODUCTION_CANARY_VERIFIED`; F10.9 conserva `STOP_REQUIRES_REBASELINE`. La [evidencia M3](operaciones/m3_f10_10_post_merge_evidence_2026_08_11.md) conserva PR #350 como antecedente sin ejecucion remota. El [rebaseline M3 reader](operaciones/m3_reader_f10_10_rebaseline.md) fija el nuevo estado local `M3_READER_REBASELINE_CANDIDATE_PENDING_PROTECTED_MERGE`; ningun gate fue consumido. Hito 1 sigue `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
| `F10.10` | `M3_READER_REBASELINE_CANDIDATE_PENDING_PROTECTED_MERGE` | [Scope M3 v2](operaciones/m3_f10_10_scope_por_ambiente_target.md) y [rebaseline M3 reader](operaciones/m3_reader_f10_10_rebaseline.md) fijan `FREE_DB` como unico target. Rol inicial `NOLOGIN/PASSWORD NULL/rolvaliduntil NULL`; activacion Q0 privada, provisioner PostgreSQL 17 cerrado, predecessor Q0 obligatorio y teardown quarantine-first. PR #350 queda antecedente v1. Candidate y CI/post-merge pendientes; cero acceso/DDL remoto y cero gates consumidos. |
...
- Subfase tecnica activa: F10.10 `M3_READER_REBASELINE_CANDIDATE_PENDING_PROTECTED_MERGE`. F10.9 conserva `STOP_REQUIRES_REBASELINE`; G5-G13, Certification, Pro, M4-M10, schedules, observacion y F11.1 siguen suspendidos. La preparacion local no concede llamadas remotas, DDL ni manejo de passwords.
...
- Siguiente accion: fusionar por PR protegido el candidate completo descrito en [M3 reader rebaseline](operaciones/m3_reader_f10_10_rebaseline.md) y exigir CI/post-merge. Solo despues podra revisarse un payload exacto para los gates propuestos `APPROVE_F10_10_M3_READER_PREFLIGHT_FREE`, `APPROVE_F10_10_M3_READER_DDL_FREE`, `APPROVE_F10_10_M3_READER_Q0_FREE`, el gate existente `APPROVE_M3_FREE_READONLY` y `APPROVE_F10_10_M3_READER_TEARDOWN_FREE`. Todos son no consumidos/no ejecutables en este corte.
- Seguridad local: una credencial usada previamente en un canary local requiere rotacion/revocacion fuera de banda y no puede reutilizarse. No se registra su valor; el cierre requiere atestacion sanitizada posterior.
