...
| `F10.9` | `REBASELINED_FG2_FG3_OPERATIONAL_REMEDIATION` | PR #384, que corresponde exclusivamente a PR A, queda `MERGED_POST_MERGE_VERIFIED`: candidate `9414480b6cb6496fc978b7379a5b24a1ae9e1f60`, merge protegido `7a4c6420214dd1ffcc367b1f35cb5f553d07c99c` / tree `4c647e87a4effbc577d1653fd023375c2c87fa3e`; Security `31899873186=PASS`, focused trust-plane `95048814844=PASS`, F9.7 run `31899873143=PASS` / job `95048918881=PASS`, `run_attempt=1`. PR A queda congelado en esa base. PR B permanece `REPOSITORY_ONLY_TRUST_BROKER_PR_B_LOCAL_CANDIDATE`, no fusionado ni verificado post-merge; [ADR-0013](decisiones/ADR-0013_trust_broker_durable_object_ledger.md) define Worker trust broker, JWT/JWKS offline, adapter GitHub App read-only con fixtures y un Durable Object coordinador que serializa por identidad gate/replay/CAS `ABSENT -> READY -> CONSUMED`, expiracion y receipt sanitizado. No hay deployment, Cloudflare remoto, GitHub App configurada, Production, Supabase, SQL, writers, schedules ni red live. Gate `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED`; trust `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`; connected `STOP_G5_CONNECTED_MODE_NOT_IMPLEMENTED`. `G4=PASS_CA1_FG2_FG3_ONLY_METADATA_TRANSFERRED_TO_H2`; pares `0`, observacion `NOT_STARTED`. |
...
- Siguiente accion: promover mediante PR protegido el PR B repository-only del trust broker y ledger G5. Aun con broker/ledger probados offline, trust operacional permanece `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`; gate `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED`; connected mode `STOP_G5_CONNECTED_MODE_NOT_IMPLEMENTED`. Deployment, configuracion Cloudflare/GitHub App, Production, API/JWKS live, workflow manual, protection rule, SQL, writers y schedules permanecen fuera.

## Tracking Only

Esta seccion es `TRACKING_ONLY`: no modifica estados, gates, criterios, evidencia ni autoridad ejecutable. Hito 1 contractual se informa como `60%`; F10.9 como `38%`; G5 end-to-end como `30%`. Formula: avance ponderado observado = componentes repository-only verificados / componentes end-to-end requeridos. Denominadores informativos: Hito 1 `3/5` bloques CA1 mayores, F10.9 `3/8` gates operativos G5-G13 reabiertos con G4 historico fuera del denominador, G5 `3/10` dominios end-to-end (estructura GET-only, routing real y trust-plane modelado; quedan ledger atomico, workflow real, OIDC live, approval real, Production, connected transport y observacion). La actualizacion solo procede cuando un PR protegido post-merge tenga Security, F9.7 y focused PASS y el Context Graph siga sin drift.
