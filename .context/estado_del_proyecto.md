...
| `FASE-07` | G1b minimo | `PENDING` | Siguiente fase autorizable; package F6 permanece `reconciled_not_certified`. |
...
- Fase autorizada: ninguna; F6 esta cerrada.
- Siguiente fase autorizable: `FASE-07`, solo con autorizacion humana explicita.

## Alcance Inmediato

`FASE-06` produjo migrations por postcondicion, un manifest cerrado y la exclusion mecanica de `H-00`. PR #223 incorporo el candidate y PR #224 corrigio la identidad LF/CRLF del manifest y ledger. Ambos recibieron CI y aprobacion independientes; `desarrollo@e00052a` conserva el tree exacto del forward-fix y pasa 74 pruebas desde Docker sobre Windows. F6 queda cerrada sin aplicar migrations en Free o Pro. `H1-CA7P` esta completado; `H1-CA1` y `H1-CA2P` mantienen `TASK-H1-001` en progreso y pasan a F7 solo mediante una autorizacion nueva.
