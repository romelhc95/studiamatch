...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.10 queda `M3_PUBLIC_DB_ACL_DIAGNOSTIC_STOP_BINDING_REQUIRED`: el gate diagnostico v1 fue consumido en STOP pre-red por bindings nulos. El candidate sucesor se prepara sin acceso remoto. Q0, lectura y teardown no consumidos. F10.9 conserva `STOP_REQUIRES_REBASELINE`; Hito 1 sigue `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
| `F10.10` | `M3_PUBLIC_DB_ACL_DIAGNOSTIC_STOP_BINDING_REQUIRED` | El gate diagnostico fue consumido una vez y termino `STOP_CANDIDATE_BINDING_PENDING` antes de red porque candidate commit/tree seguian nulos. Hubo cero llamadas, retries, transacciones o lecturas. DDL v1/v2 permanecen consumidas/fallidas; Q0, lectura funcional y teardown no fueron consumidos. |
...
- Subfase tecnica activa: F10.10 `M3_PUBLIC_DB_ACL_DIAGNOSTIC_STOP_BINDING_REQUIRED`. El gate diagnostico fue consumido en STOP pre-red; requiere nuevo candidate protegido, payload y gate, sin reutilizar esta identidad.
...
- Siguiente accion: promover primero un candidate protegido y preparar un payload sucesor con commit/tree no nulos. `APPROVE_F10_10_M3_PUBLIC_DB_ACL_DIAGNOSTIC_FREE` queda consumido y no reutilizable.
