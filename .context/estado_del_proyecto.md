...
| `F10.9` | `REBASELINED_FG2_FG3_OPERATIONAL_REMEDIATION` | PR #382 integra repository-only v2.2 mediante candidate `8a6724a5850792383456763a119c925c53961f2a` y merge protegido `58e0a0b37f7a3795e9487ab01aa558b5ecaa6ae3` / tree `13eb0465233c9e870995763630ee9e6541a45add`; Security Audit `31861308128=PASS`, F9.7 `31861308133=PASS` y focused `94955078030=159 PASS`. Su resultado exclusivo es `MERGED_POST_MERGE_CI_PASS_ROUTING_REMEDIATION_REQUIRED`. El [contrato offline GET-only](operaciones/g5_get_only_adapter_contract_2026_08_14.md) v2.2 queda `HISTORICAL_ANTECEDENT_NOT_FIT_FOR_CONNECTED_MODE`; el candidate local v2.3 queda `REMEDIATED_REPOSITORY_ONLY_V2_3_TRUST_STOP`, pendiente de promocion en un nuevo PR. V2.3 usa `SourceAttemptResult` y decision causal en el earliest valid source attempt, con cierre de snapshot 1 solo sin bundles; canonicaliza/deduplica URL, limita FILTER regex a text 2000 con subset lineal que rechaza agrupacion/alternancia/cuantificadores no escapados, rechaza localhost/IP no global y deja `circuit_opened_at` dormido fingerprint-only. FG3 exige `category_counts=3` y `courses/prior_mutations/history<=50000`; `65/50001 -> STOP_G5_TARGET_BINDING_INVALID`. Conserva cooldown 24h, exact-one, blockers separados y frontera static-only/dynamic FG2. La divergencia NULL del preflight no sustituye routing harvester; preflight y workers no cambian. Termina `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`. Gate `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED`; connected mode `STOP_G5_CONNECTED_MODE_NOT_IMPLEMENTED`. `G4=PASS_CA1_FG2_FG3_ONLY_METADATA_TRANSFERRED_TO_H2`; pares `0`, observacion `NOT_STARTED`. |
...
- Siguiente accion: promover en un nuevo PR protegido el candidate local repository-only v2.3 del [contrato offline GET-only](operaciones/g5_get_only_adapter_contract_2026_08_14.md), bajo autorizacion separada. Aun con evidencia estructural valida, v2.3 termina `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`; el gate G5 permanece `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED` y connected mode permanece `STOP_G5_CONNECTED_MODE_NOT_IMPLEMENTED`. No implementar ni ejecutar autoridad confiable, consumo de gate, replay ledger, Production, adapter de red, schedules u observacion.
