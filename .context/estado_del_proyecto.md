...
| `F10.9` | `REBASELINED_FG2_FG3_OPERATIONAL_REMEDIATION` | PR #380 queda `MERGED_POST_MERGE_CI_PASS_REMEDIATION_REQUIRED` en `desarrollo@c7783af918c4e434d31b80e9a65247329c0b3595` / tree `37d4ab05738355436169188d2613f860c6b35148`; Security Audit `31848341499=PASS` y F9.7 `31848341110=PASS`. El [contrato offline GET-only](operaciones/g5_get_only_adapter_contract_2026_08_14.md) v2 queda antecedente historico no apto para connected mode; el sucesor repository-only v2.1 liga duracion source, strict Boolean y errores estables de dataclasses incompletas, y termina `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`. Gate `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED`; connected mode `STOP_G5_CONNECTED_MODE_NOT_IMPLEMENTED`. `G4=PASS_CA1_FG2_FG3_ONLY_METADATA_TRANSFERRED_TO_H2`; pares `0`, observacion `NOT_STARTED`. |
...
- Siguiente accion: revisar y promover por PR protegido la remediacion repository-only v2.1 del [contrato offline GET-only](operaciones/g5_get_only_adapter_contract_2026_08_14.md). Aun con evidencia estructural valida termina `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`; el gate G5 permanece `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED` y connected mode permanece `STOP_G5_CONNECTED_MODE_NOT_IMPLEMENTED`. No implementar ni ejecutar autoridad confiable, consumo de gate, replay ledger, Production, adapter de red, schedules u observacion.
