...
Snapshot: `SNAPSHOT-2026-08-10-F10.10-M0-REBASELINE-CANDIDATE`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.8 queda `COMPLETED_PRODUCTION_CANARY_VERIFIED`; F10.9 conserva `STOP_REQUIRES_REBASELINE`. PR #342 fue fusionado en `desarrollo@909b9cbd451e76777ec70df6f56675d6fb563199` con Security Audit `31412721559=PASS` y F9.7 `31412721306=PASS`. [ADR-0010](decisiones/ADR-0010_rebaseline_f10_10_metadata_remediation.md) crea F10.10 para remediacion metadata separada con umbral cero preservado. M0 permanece pendiente de merge protegido de este package; no autoriza M1 ni acceso remoto. Hito 1 sigue `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
| `F10.10` | `IN_PROGRESS_M0_AUTHORITY_REGISTRATION` | [ADR-0010](decisiones/ADR-0010_rebaseline_f10_10_metadata_remediation.md) y [PLAN-F10.10-001](operaciones/plan_remediacion_metadata_f10_10.md) definen una fase mutante separada, fill-only y sin DDL. M0 solo registra autoridad; M1-M10 requieren autorizaciones separadas. F10.9 no se reactiva automaticamente. |
...
- Subfase tecnica activa: F10.10 `IN_PROGRESS_M0_AUTHORITY_REGISTRATION`. F10.9 conserva `STOP_REQUIRES_REBASELINE`; G5-G13, schedules y observacion siguen suspendidos. M0 es exclusivamente documental y no autoriza M1, Supabase, providers, environments ni writers.
...
- Siguiente accion: aprobar y fusionar el package documental M0 de F10.10, verificar checks post-merge y reconciliar M0. Solo despues puede solicitarse autorizacion decimal F10.10 para M1 local; ningun acceso remoto queda concedido.
