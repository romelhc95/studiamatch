...
| `F9` | Certificacion Hito 1 CA1-only | `IN_PROGRESS` | F9.9 queda cerrada documentalmente con PR #277 en `certificacion`, desviacion `DEVIATION_ACCEPTED_FAIL_CLOSED`, controles pre-main PR #280 mergeados en `desarrollo`, PR #282 mergeado en `certificacion@bc227629b8df1fcabca47ea7be3ea1d5b4c7667b` y QA independiente `PASS`; F9.10 ejecuta readiness de repositorio antes de F10. No hay canary Certification positivo, Production ni schedules. |
...
| `F9.10` | `ACTIVE_AUTHORIZED_IN_PROGRESS` | Readiness de repositorio autorizada por frase decimal exacta: gate CI `f10-main-boundary`, canary Production definido pero no ejecutado, rollback/snapshot privado, correccion `DB-SYNC` vs writers, registro sanitizado de PR #282/run `30824041542`, subfases F10.x y validaciones locales. `USER_PERSONAL_UAT` sigue pendiente antes de readiness F10. |
...
- Siguiente accion futura: completar validaciones F9.10, PR a `desarrollo`, reconstruccion selectiva a `certificacion`, `USER_PERSONAL_UAT` y solo entonces readiness F10. F10 permanece bloqueada hasta readiness inequivoca.
