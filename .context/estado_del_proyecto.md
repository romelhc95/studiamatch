...
Snapshot: `SNAPSHOT-2026-08-10-F10.10-M2-POST-MERGE`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.8 queda `COMPLETED_PRODUCTION_CANARY_VERIFIED`; F10.9 conserva `STOP_REQUIRES_REBASELINE`. [M2 F10.10 post-merge](operaciones/m2_f10_10_post_merge_evidence_2026_08_10.md) registra PR #345/#346 y checks post-merge PASS: M1 queda integrado offline y M2=`PASS`. M3-M10 requieren autorizaciones separadas. Hito 1 sigue `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`. |
...
| `F10.10` | `M2_PASS_M3_SEPARATE_AUTH_REQUIRED` | [ADR-0010](decisiones/ADR-0010_rebaseline_f10_10_metadata_remediation.md), [PLAN-F10.10-001](operaciones/plan_remediacion_metadata_f10_10.md) y la [evidencia M2](operaciones/m2_f10_10_post_merge_evidence_2026_08_10.md) preservan la frontera fill-only y sin DDL. M1/M2 quedaron integrados y verificados; M3-M10 requieren autorizaciones separadas y F10.9 no se reactiva automaticamente. |
...
- Subfase tecnica activa: F10.10 `M2_PASS_M3_SEPARATE_AUTH_REQUIRED`. F10.9 conserva `STOP_REQUIRES_REBASELINE`; G5-G13, schedules y observacion siguen suspendidos. M3 requiere autorizacion separada por ambiente/target; no hay permiso vigente para Supabase, providers, environments, writers ni workflows operativos.
...
- Siguiente accion: definir y aprobar M3 por ambiente y target fisico para diagnostico read-only. Hasta entonces no se ejecuta acceso remoto, provider, writer, backup/restore, DML, schedule ni observacion.
