...
Snapshot: `SNAPSHOT-2026-08-07-F10.8-DB-SYNC-REMEDIATED-TECHNICAL-CLOSURE`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS` | F10.8 queda `DB_SYNC_REMEDIATED_PRODUCTION_CANARY_PENDING`. La remediacion fail-closed de DB Sync fue promovida por PR #304 a `desarrollo`, PR #305/#306 a `certificacion` y PR #307 a `main@529ca111f1fef40efb15676ad6f07d002a54ae92`. `DB Sync to Production` run `31151066062=SUCCESS_NO_DB_CHANGES_SKIPPED`: solo corrio `Detect DB changes`; `DB contract preflight`, `Report pending migrations`, `Apply pending migrations`, `Verify target schema` y `FG2 deferred` quedaron skipped por ausencia de cambios `db/**`. `Security Audit Gate` post-main run `31151066061=PASS`. No hubo Supabase, DDL/DML, migrations, Production Canary, schedules, writer ni mutacion DB. Hito 1 queda `TECHNICALLY_DELIVERED_FORMAL_CLOSURE_PENDING`; `EVID-H1-010..013/016` siguen pendientes. |
...
| `F10.8` | `FG1_SOURCE_SLUG_FIX_LOCAL_VALIDATED` | Remediacion Production Canary promovida por PR #297 a `main@260900a268ab8eb194140ea7311aec2a170b6e17`; Certification Canary `31140933096=PASS` sobre `certificacion@94026de77fe9c1a01c66eae78bea8b09858daf96`, manifest/artifacts sanitizados y conteos pre/post/after-cleanup sin cambio. Incidente DB Sync `31142826000=FAIL_CLOSED_PRE_SUPABASE` remediado por PR #304/#305/#306/#307; post-merge `main@529ca111f1fef40efb15676ad6f07d002a54ae92`, `DB Sync to Production` `31151066062=SUCCESS_NO_DB_CHANGES_SKIPPED` y `Security Audit Gate` `31151066061=PASS`. Remediacion source-preflight/sanitizacion promovida por PR #309/#310/#311 y ajuste F9.7 por PR #312 hasta `main@32526efadc21b734c58e47ff00f3a5be5b042f24`. Production Canary `31157736479=FAIL_CLOSED_HTTP_403_RESTORE_NOOP`; segundo Production Canary `31223623363=FAIL_CLOSED_FG1_EXIT1_RESTORE_NOOP`: target/candidate/limites/source-access preflight/snapshot PASS, FG1 fallo con exit `1`, FG2/FG3 skipped, restore exacto y segundo restore NOOP PASS. Diagnostico local identifico acoplamiento indebido entre slug FG1 y cohorte FG2/FG3; correccion local separa `F10_PRODUCTION_CANARY_FG1_SOURCE_SLUG` de `F10_PRODUCTION_CANARY_INSTITUTION_SLUG` y queda validada en Docker. `EVID-H1-010..013/016=PENDING`; promocion protegida, retry completo, schedules y cierre final requieren autorizaciones separadas. |
| `F10.9` | `PENDING` | Habilitacion gradual de schedules y observacion: al menos 72h y tres pares FG2 -> FG3 consecutivos completos. |

Base tecnica post-source-preflight-remediation: `main@32526efadc21b734c58e47ff00f3a5be5b042f24`.
...
- Subfase tecnica activa: F10.8 `FG1_SOURCE_SLUG_FIX_LOCAL_VALIDATED`; PR #311 promovio la remediacion source-preflight/sanitizacion a `main@32526efadc21b734c58e47ff00f3a5be5b042f24`. Segundo Production Canary `31223623363` valido target/candidate/limites/source-access preflight/snapshot y fallo fail-closed en FG1 con exit `1`; restore exacto y segundo restore NOOP completados. La correccion local separa el secret FG1 source del secret de cohorte mutable y queda pendiente de promocion protegida a `main`. F10.7 permanece `COMPLETED_TECHNICAL_DELIVERY` y conserva [ADR-0008](decisiones/ADR-0008_rebaseline_f10_7_gate_reconstruction.md) y [ADR-0009](decisiones/ADR-0009_reconciliacion_entrega_tecnica_post_main_f10_7.md) como antecedentes.
...
- Compatibilidad contractual: el segundo Production Canary fue ejecutado con autorizacion separada y quedo fail-closed antes de FG2/FG3; no autoriza schedules, writers programados, DDL/DML, backfill ni CA2. `EVID-H1-010..013/016` siguen pendientes y requieren autorizaciones separadas para diagnostico/retry, F10.9 y F11.1. Este registro no completa Hito 1 contractual.
...
- Siguiente accion futura: promover la correccion FG1 source slug por el flujo protegido, confirmar que el secret `F10_PRODUCTION_CANARY_FG1_SOURCE_SLUG` existe sin exponer su valor y ejecutar el retry completo ya autorizado sobre el nuevo SHA exacto de `main`; F10.9 schedules y F11.1 cierre/conformidad siguen bloqueados hasta completar ese canary y sus evidencias.
