...
| `F9` | Certificacion Hito 1 en Free | `IN_PROGRESS` | Package local sucesor F9.7 queda PR-ready local; snapshot remoto, restore/pausa y autorizaciones bloquean aplicacion; Free sigue sin certificar. |
...
| `F9.7` | `IN_PROGRESS` | `LOCAL_SIX_ENTRY_TRIGGER_RETIREMENT_PR_READY_REMOTE_AND_OPERATIONAL_GATES_BLOCKED`; cinco migrations historicas intactas y aplicacion prohibida |
...
- Subfase activa: F9.7; la [remediacion local del trigger](operaciones/remediacion_trigger_f9_7.md) define un unico camino manifest-only de seis entradas, retira `trg_notify_new_lead` y `public.notify_new_lead()` sin `CASCADE`, tombstonea la Edge Function historica y preserva el package historico de cinco entradas.
- Subfase autorizada: solo definicion local. Ninguna operacion remota; la lectura ACL consumida no autoriza retry, nueva lectura, DDL/DML, backup/restore, writers, aplicacion, backfill ni Pro.
- Siguiente accion: abrir PR del package sucesor local; cualquier observacion remota o gate operativo requiere autorizacion separada.

## Alcance Inmediato

La [macrofase F9](operaciones/certificacion_hito1_f9.md) sigue el [plan simplificado](operaciones/plan_simplificado_hito1.md). F6-F8 y la closure F9.7 permanecen byte-identicas; un manifest sucesor agrega una sexta migration local de retiro del trigger. La atribucion Free previa solo cubrio fuentes ACL y no prueba el snapshot actual ni convergencia. El draft de atestacion suplementaria no fue ejecutado. Gate B y la atestacion ACL quedan consumidos; backup/restore y writers solo tienen runbooks de datos sin capacidad. Los artifacts F9.5 siguen `HISTORICAL_NON_PROMOTABLE`. F10 Produccion y F11 Cierre siguen pendientes.
