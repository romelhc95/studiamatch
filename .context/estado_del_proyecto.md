...
| `F9.7` | `IN_PROGRESS` | `security_cutoff_local_candidate`; PR #258 tuvo `NO_GO_CI` inicial, `NO_GO_CI_120D234` y `NO_GO_CI_D2CB32E`; candidates `b711e0b`/`249295`, `120d234`/`bf68ed` y `d2cb32e`/`16081a60` quedan no promocionables; `CORR-WP-F9.7-04-02` esta `IN_PROGRESS`, `WP-F9.7-05` queda `INVALIDATED_BY_CI_PARITY_GAP` y `WP-F9.7-06` continua `NO_GO_CI`; v3 byte-identico, security hold bloqueado para Free/Pro, Free/Pro `UNCHANGED_NOT_ATTESTED`, Supabase previews skipped y `AUTOMATIC_PR_PREVIEW_ACCEPTED` sin Cloudflare manual |
...
- Subfase activa: F9.7; [ADR-0005](decisiones/ADR-0005_corte_seguridad_funcionalidad_estabilidad_hito1.md), [PLAN-H1-CORTE-SFE-001](operaciones/plan_corte_seguridad_funcionalidad_estabilidad_hito1.md) y [PLAN-F9.7-CIERRE-001](operaciones/cierre_definitivo_f9_7.md) fijan el corte local y su secuencia `WP-F9.7-01..06`: PR #258 descubrio `BLOCKING_IN_SCOPE` de CI/release gates propiedad de `WP-F9.7-04`; `120d234` fallo por paridad actionlint/env-i/frontend cold-cache y `d2cb32e`/`16081a60` queda `NO_GO_CI_D2CB32E_NOT_PROMOTABLE`; Frontend Static Build CI de `d2cb32e` paso, pero quedan causas actionlint/ShellCheck parity y commit-mode Git read-only. Por tanto `CORR-WP-F9.7-04-01=NO_GO_CI_PARTIAL`, `CORR-WP-F9.7-04-02=IN_PROGRESS`, `WP-F9.7-05=INVALIDATED_BY_CI_PARITY_GAP`, `WP-F9.7-06=NO_GO_CI` y las auditorias previas quedan `INVALIDATED_STALE_ORACLE`; los dos `SC2086` legacy quedan `RESIDUAL_ACCEPTED_PROTECTED_BASELINE`.
