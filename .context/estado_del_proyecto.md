...
Snapshot: `SNAPSHOT-2026-08-13-F10.10-M3-PUBLIC-ACL-PREFLIGHT-GATE-PENDING`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.10 queda `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_POST_MERGE_VERIFIED_GATE_PENDING`: PR #369 fue promovido mediante merge protegido y CI post-merge PASS; el gate Free v2 sigue propuesto, no creado, no aprobado y no consumido. |
...
| `F10.10` | `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_POST_MERGE_VERIFIED_GATE_PENDING` | PR #369 aprobado y fusionado en `desarrollo@6068f2ac9ef623e06dcc23d9828980641e396c39` / tree `6a7ebe58b0acdc79bafe8362239c797e7256e31f`; Security Audit `31701896740=PASS` y contrato PostgreSQL 17 `31701896698=PASS`. El gate bound historico permanece `CONSUMED_ONCE`; el gate Free v2 permanece `PROPOSED_NOT_CREATED_NOT_APPROVED_NOT_CONSUMED`. |
...
- Subfase tecnica activa: F10.10 `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_POST_MERGE_VERIFIED_GATE_PENDING`. El candidate esta promovido y verificado; la siguiente capacidad exige exclusivamente aprobacion humana separada del gate Free v2.
...
- Siguiente accion: solicitar por separado `APPROVE_F10_10_M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_FREE_V2`. El gate sigue propuesto, no creado, no aprobado y no consumido; remediacion, postflight y reader v3 permanecen bloqueados.
