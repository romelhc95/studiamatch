...
| `F9` | Certificacion Hito 1 en Free | `IN_PROGRESS` | Candidate local F9.7 v3 queda PR-ready local; snapshot remoto, restore/pausa y autorizaciones bloquean aplicacion; Free sigue sin certificar. |
...
| `F9.7` | `IN_PROGRESS` | `v3_local_pr_ready`; cinco migrations historicas intactas, T02 `NOT_EXECUTED`, backup `PLANNED`, writers `INVENTORIED`, Free/Pro `UNCHANGED_BLOCKED` y aplicacion prohibida |
...
- Subfase activa: F9.7; la [remediacion local del trigger](operaciones/remediacion_trigger_f9_7.md) define v3 como unico successor local promocionable de seis entradas, retira `trg_notify_new_lead` y `public.notify_new_lead()` sin `CASCADE`, tombstonea la Edge Function historica y preserva v2 como antecedente historico no promocionable.
- Subfase autorizada: solo definicion local y PR GitHub tras GO local. Ninguna operacion remota; la lectura ACL consumida no autoriza retry, nueva lectura, DDL/DML, backup/restore, writers, aplicacion, backfill ni Pro.
- Siguiente accion: `merge_and_postmerge_replay_then_define_PR_O`; PR-O definira el contrato sucesor F9.7 v3, hold DB ACL y gates operativos sin reactivar el T02 historico F10.

## Alcance Inmediato

La [macrofase F9](operaciones/certificacion_hito1_f9.md) sigue el [plan simplificado](operaciones/plan_simplificado_hito1.md). F6-F8 y la closure F9.7 permanecen byte-identicas; el manifest sucesor v3 agrega una sexta migration local de retiro del trigger. La atribucion Free previa solo cubrio fuentes ACL y no prueba el snapshot actual ni convergencia. El draft de atestacion suplementaria no fue ejecutado. Gate B y la atestacion ACL quedan consumidos; backup/restore y writers solo tienen runbooks de datos sin capacidad. Los artifacts F9.5 siguen `HISTORICAL_NON_PROMOTABLE`. F10 Produccion y F11 Cierre siguen pendientes.
