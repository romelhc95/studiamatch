...
Snapshot: `SNAPSHOT-2026-08-13-F10.10-M3-PUBLIC-ACL-FINAL-READINESS-CANDIDATE`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.10 queda `M3_PUBLIC_DB_ACL_FINAL_READINESS_RACE_REMEDIATION_CANDIDATE_PENDING_PROMOTION_CONSUMER_BINDING_REQUIRED`: PR #373 promovio la evidencia, pero F9.7 post-merge `31724004476` fallo por carrera local de init. El candidate actual corrige solo readiness; consumer binding sigue pendiente. |
...
| `F10.10` | `M3_PUBLIC_DB_ACL_FINAL_READINESS_RACE_REMEDIATION_CANDIDATE_PENDING_PROMOTION_CONSUMER_BINDING_REQUIRED` | PR #373 promovio la evidencia en `desarrollo@51dac8f4906725aeb9d11172e674eafb5df87b8b` / tree `0382efc31ea3540ac8efa82046210520cd7da1a4`; Security Audit `31724004474=PASS`, pero F9.7 `31724004476=FAIL_CLOSED_LOCAL_POSTGRES_INIT_RACE` tras 34 pruebas Python PASS. El candidate actual corrige solo readiness del PostgreSQL local definitivo. Consumer binding sigue `REQUIRED_NOT_IMPLEMENTED`. |
...
- Subfase tecnica activa: F10.10 `M3_PUBLIC_DB_ACL_FINAL_READINESS_RACE_REMEDIATION_CANDIDATE_PENDING_PROMOTION_CONSUMER_BINDING_REQUIRED`. La remediacion actual no consume el gate ni abre red; el consumer binding permanece fuera del candidate.
...
- Siguiente accion: promover el readiness estable por PR protegido y exigir Security Audit/F9.7 post-merge PASS. Solo despues vuelve a habilitarse la preparacion del consumer candidate separado. El gate sigue no creado, no aprobado y no consumido.
