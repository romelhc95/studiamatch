...
Snapshot: `SNAPSHOT-2026-08-13-F10.10-M3-PUBLIC-ACL-PREFLIGHT-V2-PAYLOAD`.
...
| `F10` | Produccion CA1-only | `IN_PROGRESS_REBASELINE_METADATA` | F10.10 queda `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_FREE_V2_PAYLOAD_CANDIDATE_PENDING_PROMOTION_CONSUMER_BINDING_REQUIRED`: payload sanitizado candidate preparado; falta promoverlo y adaptar el consumidor para binding canónico target/transport antes de aprobación o consumo. |
...
| `F10.10` | `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_FREE_V2_PAYLOAD_CANDIDATE_PENDING_PROMOTION_CONSUMER_BINDING_REQUIRED` | PR #370 post-merge PASS. Payload v2 null-bound candidate preparado sobre `desarrollo@f78713087813bea950e320bc37c55cdd36c95a70`. No es ejecutable: falta merge/CI del payload y un consumer candidate separado que valide `target-binding-v2` y `observed-transport-v2` de la misma conexion. |
...
- Subfase tecnica activa: F10.10 `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_FREE_V2_PAYLOAD_CANDIDATE_PENDING_PROMOTION_CONSUMER_BINDING_REQUIRED`. La preparacion no consumio el gate ni abrio red; primero debe promoverse el payload y luego implementarse/promoverse el consumer binding canónico.
...
- Siguiente accion: reponer fuera de Git `F10_10_M3_API_URL`, `F10_10_M3_PROJECT_REF`, `F10_10_M3_SQL_HOST`, `F10_10_M3_CA_SHA256`, provisioner y ventana; luego generar `target-binding-v2`, exigir `observed-transport-v2` de la misma conexion y promover binding sucesor. El gate sigue no aprobado y no consumido.
