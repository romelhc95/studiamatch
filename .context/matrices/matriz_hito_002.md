...
| `H2-CA3` | No publicacion automatica | `PASS_IN_DEVELOPMENT` | Los cursos nuevos pendientes fuera de la cohorte no aparecen; la cohorte legacy solo conserva lo que ya era visible por negocio. | Nada nuevo incompleto se publica por accidente y la web no queda vacia si hay cursos elegibles. |
| `H2-COMPAT` | Transparencia funcional Desarrollo | `PASS_IN_DEVELOPMENT` | `private.h2_legacy_public_course_cohort` y vista H2 compatible; sin fallback frontend a `courses`. | La web sigue mostrando cursos mientras el equipo editorial completa la revision. |
