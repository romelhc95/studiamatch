...
| `H2-PRO` | Preparacion productiva | `PRO_EXPAND_APPLIED_VERIFY_PENDING` | Pro apply `h2-expand-compat` ejecutado de forma aditiva con backup/PITR verificado; baseline elegible `224`; advisors pendientes de verificar con `DB Sync to Production` `operation=verify`. | Antes de produccion se prepara la base para que el catalogo no desaparezca y se valida sin hallazgos HIGH/CRITICAL. |
| `H2-POST-477` | Endpoint Security Advisor | `MERGED_TO_DESARROLLO_AND_CERTIFICATION` | PR #477 usa endpoint soportado por Supabase para advisors. | Se mantiene la capacidad de auditar advisors sin dependencias inestables. |
| `H2-POST-478` | RLS cohorte privada | `MERGED_TO_CERTIFICATION` | PR #478 aplica RLS sobre `private.h2_legacy_public_course_cohort`. | Los datos privados de la cohorte legacy quedan protegidos. |
| `H2-POST-480-481` | CI DB Sync verify | `MERGED_TO_DESARROLLO_AND_CERTIFICATION` | PRs #480/#481 ajustan `db-sync-to-pro.yml` para no-op apply success bajo `operation=verify`. | El gate H2 de `main` puede recibir artifact valido despues de un apply previo. |
