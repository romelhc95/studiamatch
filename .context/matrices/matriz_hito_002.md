...
| `H2-CA3` | No publicacion automatica | `PASS_IN_DEVELOPMENT_LOCAL` | Los cursos nuevos pendientes fuera de la cohorte no aparecen; la cohorte legacy solo conserva lo que ya era visible por negocio. | Nada nuevo incompleto se publica por accidente; la web real aun requiere apply Free para recuperar cursos. |
| `H2-COMPAT` | Transparencia funcional Desarrollo | `NO-GO_PENDING_FREE_MIGRATION` | Preflight Free: `227` legacy elegibles, `0` H2 estrictos, `0` efectivos; falta aplicar `private.h2_legacy_public_course_cohort`. | No se puede cerrar funcionalidad hasta que la web real muestre los cursos elegibles. |
