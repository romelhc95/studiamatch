...
| Hito 1 (`H1-CA1`, `H1-CA2P`, `H1-CA7P`) | Free | Gate B FAIL; origen ACL atestado read-only | Fuentes observadas reparables; snapshot actual no observado | Candidate v3 local F6-F8 + closure + retiro de trigger; security hold local bloqueado; [PR-O v3 + hold](./pr_o_f9_7_v3_hold.md) definido sin autorizacion; runbooks no ejecutables | `observed_effective_unledgered` | No DDL; aprobar GO_FOR_FREE independiente antes de cualquier gate remoto o aplicacion |
...
El [contrato PR-O F9.7 v3 + hold](./pr_o_f9_7_v3_hold.md) liga localmente v3 y el security hold como unidad Free-only futura. Su estado es `DEFINED_LOCAL_NOT_AUTHORIZED`, `application_authorized=false`, `capabilities=[]`, boundaries `0/3/4/5/6/7`, con target binding privado, snapshot read-only, restore/writers/window pendientes y cero capacidad remota ejecutable. No cambia ledger ni postcondicion observada de Free o Pro.
