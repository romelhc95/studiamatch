...
| Hito 1 (`H1-CA1`, `H1-CA2P`, `H1-CA7P`) | Free | Gate B FAIL; origen ACL atestado read-only | Fuentes observadas reparables; snapshot actual no observado | Candidate v3 local F6-F8 + closure + retiro de trigger; hold actual y [PR-O v1](./pr_o_f9_7_v3_hold.md) `SUPERSEDED_NON_PROMOTABLE`; [PR-O executor privado](./pr_o_f9_7_successor_private_executor.md) `GO_WP_LOCAL`; runbooks no ejecutables | `observed_effective_unledgered` | No DDL; certificar localmente el PR-O sucesor antes de cualquier GO_FOR_FREE o gate remoto |
