...
| Hito 1 (`H1-CA1`, `H1-CA2P`, `H1-CA7P`) | Free | Gate B FAIL; remediacion local bloqueada por atribucion ACL | Drift directo cubierto localmente; origen efectivo remoto pendiente | Package F6-F8 + closure F9.7 intacto; runbooks no ejecutables | `observed_effective_unledgered` | No DDL; atestacion read-only y autorizacion nuevas |
...
La [definicion de remediacion](./remediacion_gate_b_f9_7.md) no cambia la adopcion: demuestra reparacion local de clases directas y rollback de drift no atribuible, pero mantiene `observed_effective_unledgered` hasta una nueva atestacion read-only del origen ACL. Los runbooks permanecen sin aprobar y no ejecutan operaciones.
