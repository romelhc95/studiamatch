...
| `G5` | `DEPLOYMENT_READY_DISABLED_NOT_CONFIGURED` | PR #384 corresponde solo a PR A y queda `MERGED_POST_MERGE_VERIFIED`: candidate `9414480b6cb6496fc978b7379a5b24a1ae9e1f60`, merge `7a4c6420214dd1ffcc367b1f35cb5f553d07c99c` / tree `4c647e87a4effbc577d1653fd023375c2c87fa3e`; Security `31899873186=PASS`, focused `95048814844=PASS`, F9.7 run `31899873143=PASS` / job `95048918881=PASS`, attempt 1. PR #385, PR B, queda `MERGED_POST_MERGE_VERIFIED`: candidate `09851459f2ffa751e2e8139d4e9a3304427f1ee4`, merge `191539de71cbff95552c476463305e8d6f3e4b73` / tree `7fe13bb907053f4dea51ac593b5df0de78cb40d6`; Security `31903582727=PASS`, trust broker `95057934429=PASS`, F9.7 run `31903582617=PASS` / job `95058032300=PASS`, attempt 1. PR #386, PR C, queda `MERGED_POST_MERGE_VERIFIED`: candidate `d6e4eaae058b52aacf5099c763204a1343a6eebf`, merge `74defb6326d8432bf790cb84b4aa549fefc425be` / tree `b9b4cc8a6f8279f898b2b8bf2a900c56a741b528`; Security `31905626274=success`, focused `95062812645=F10.9 G5 Workflow PR C Repository-Only success`, F9.7 run `31905626285=success` / job `95062903177=F9.7 Release Gate Contract success`, attempt 1. PR #387, PR D, queda `MERGED_POST_MERGE_VERIFIED_WITH_INFRA_RETRY`: candidate `d62c8969e7d229bb8d2a9e1f8c6db6a1c4ef4d1d`, merge `bd0d82864c26755435e551b835d145b864383810` / tree `135af5a95237a1d4d6e1b977e8bb9ab82ac95e16`; Security `31912540519=PASS`, focused PR D `95079685172=PASS`, M3 `95079685191=PASS`, F9.7 run `31912540528`, attempt 1 job `95079764790=CANCELLED` clasificado `CI_INFRA_TIMEOUT_PLAYWRIGHT_APT`, attempt 2 job `95084155346=PASS` clasificado `CI_RETRY_PASS`, `run_attempt=2` solo CI. PR D define [ADR-0015](../decisiones/ADR-0015_g5_deployment_ready_disabled.md): workflow manual deployment-ready apagado por `G5_TRUST_OPERATIONAL_ENABLED`, OIDC/broker client inyectables, receipt single-use y collector Supabase GET-only publishable-only. PR E agrega [ADR-0016](../decisiones/ADR-0016_g5_operational_activation_gates.md), runbook E1-E6 y manifest name-only, sin ejecutar gates. Gate real `NOT_CREATED_NOT_APPROVED_NOT_CONSUMED`; trust `STOP_G5_TRUST_VERIFICATION_NOT_IMPLEMENTED`; connected `IMPLEMENTED_DISABLED_NOT_CONFIGURED`; futuro G5 operacional mantiene `run_attempt=1` obligatorio; cero workflow ejecutado, deployment, configuracion remota, Supabase live, SQL, writers o schedules. |
...
Nota PR O: PR #398 queda `MERGED_POST_MERGE_VERIFIED_TRUSTED_ATTESTATION_MISSING_DEFAULT_BRANCH_REGISTRATION_REQUIRED` con candidate `d03ee28ce90abcbf8efd7c4b37de99b72717207e`, base `9a5fcf539c69b635a41616e52716c0ee34837df4`, merge protegido `85d7f647a37dc784fe16c11da0318956e255b698` / tree `91706dfcc3766fbf69b4fb8c893318786445a2a9`; Security `31992887172=PASS`, `security-audit` job `95279485661=PASS`, F9.7 `31992887025=PASS`, F9.7 job `95279525942=PASS`, focused G5 `95279414529=PASS`, M3 `95279414473=PASS`, trusted check=`NOT_EXECUTED`. Root cause: default_branch=main, workflow existe en desarrollo, workflow no existe en main, `pull_request_target` requiere archivo en default branch, `edited`/retry/API enable no corrigen la ausencia y PR #398 no puede acreditarse retroactivamente como merge-gated. PR O prepara perfil PR P fail-closed y promocion selectiva `desarrollo -> certificacion -> main` no ejecutada. E2-E6 siguen `NOT_EXECUTED` con `E2_STOP_DEFAULT_BRANCH_TRUSTED_WORKFLOW_REGISTRATION_REQUIRED`. Hito 1 `60%`, F10.9 `38%`, G5 `50%`, readiness `75%` y cierre formal `NOT_READY` no cambian.

Nota PR Q: PR #399 queda `MERGED_POST_MERGE_VERIFIED` con candidate `2e7422e9f67e91ee6b02b4b44fccc060248c13a3`, base `85d7f647a37dc784fe16c11da0318956e255b698`, merge protegido `ab5b0dffe8fe7d677c083e258e86f590d393b731` / tree `fb0b0166b67a58cab14dd0c20e89f034a8adab6e`; Security `31998458176=PASS`, attempt `1`; `security-audit` job `95294350579=PASS`; F9.7 run `31998458172=PASS`, attempt `1`; F9.7 job `95294383627=PASS`; focused G5 job `95294259790=PASS`; M3 job `95294259769=PASS`. PR Q queda limitado a hardening repository-only del trusted check permanente antes de promocion: check estable `F10.9 Trusted Boundary v1`, sin `paths`/`paths-ignore`, `OUT_OF_SCOPE_SAFE` para deltas no sensibles, fail-closed para workflows, trusted-validator, superficies F10.9/trust sin perfil, forks, stale base, SHA invalido, metadata inconsistente y mode drift. El manifiesto anterior de promocion queda `SUPERSEDED_NOT_EXECUTABLE`; la promocion definitiva `desarrollo -> certificacion -> main` queda preparada y no ejecutada. E2-E6 siguen `NOT_EXECUTED` con `E2_STOP_DEFAULT_BRANCH_TRUSTED_WORKFLOW_REGISTRATION_REQUIRED`. Hito 1 `60%`, F10.9 `38%`, G5 `50%`, readiness `75%` y cierre formal `NOT_READY` no cambian.
...
siguen vinculantes. PR D agrega forma deployment-ready deshabilitada sin consumir
gate ni red live. Trust conserva
