...
9. Conservar el candidate local contractual F9.7 v3 byte-identico de seis entradas, el security hold terminal local bloqueado de [ADR-0005](../decisiones/ADR-0005_corte_seguridad_funcionalidad_estabilidad_hito1.md) y el [PR-O F9.7 v3 + hold](./pr_o_f9_7_v3_hold.md) definido localmente como Free-only sin autorizacion. Gate B termino FAIL, la atestacion ACL atribuyo fuentes reparables, v2 queda historico no promocionable, la sexta migration local retira el trigger inseguro, la Edge Function historica queda tombstoneada en Git y el drenaje pg_net queda counts-only no ejecutado; snapshot remoto y gates operativos siguen bloqueados. Antes de cualquier lectura o aplicacion se requieren gates nuevos. Resguardo-restauracion, pausa de writers y aplicacion v3 + hold siguen sin aprobar y conservan decisiones separadas.
...
- F9.7 schema/RLS/corte local, F9.8 plan de backfill, F9.9 ejecucion de backfill y F9.10 certificacion Free son gates distintos; F9.7 tiene v3 byte-identico, security hold terminal local bloqueado y PR-O combinado v3 + hold `DEFINED_LOCAL_NOT_AUTHORIZED`; sigue bloqueada por aprobacion Free independiente, snapshot remoto y gates operativos. El T02 historico F10 no se reactiva.
