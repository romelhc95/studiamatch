...
9. Conservar el candidate local contractual F9.7. Gate B pre-DDL/read-only termino `FREE_GATE_B_FAIL_STOPPED_READ_ONLY`; antes de repetirlo o remediar se requiere definicion y autorizacion nuevas. Resguardo-restauracion, pausa de writers y aplicacion schema/RLS siguen sin aprobar y conservan gates separados.
...
- F9.7 schema/RLS, F9.8 plan de backfill, F9.9 ejecucion de backfill y F9.10 certificacion Free son gates distintos; F9.7 conserva candidate local y un Gate B remoto fallido/detenido cuya autorizacion fue consumida.
