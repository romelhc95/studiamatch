...
17. F10.10 mantiene la remediacion metadata en `M3_READER_PREFLIGHT_PASS_DDL_GATE_PENDING`. PR #357 promovio el payload y el preflight local passwordless termino PASS con network none, sin password ni capacidad remota. Solo el gate preflight fue consumido; payload y gate literal DDL preceden cualquier provision. Certification, Pro, M4 y F10.9/G4 siguen bloqueados.
...
| `F10.10` | `M3_READER_PREFLIGHT_PASS_DDL_GATE_PENDING` | [Scope v2 Free-only](./m3_f10_10_scope_por_ambiente_target.md): preflight local passwordless PASS sin red/password/capacidad remota. Gate preflight consumido una vez; DDL, Q0, lectura y teardown no consumidos. NOLOGIN inicial, aislamiento Pro y teardown quarantine-first permanecen obligatorios. |
