...
17. F10.10 queda `M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_CANDIDATE_PENDING_PROMOTION`: el diagnostico bound fue `CONSUMED_ONCE` mediante un unico `execute_sql`, sin retry y read-only sobre PostgreSQL 17; termino `STOP_PUBLIC_DB_ACL_REMEDIATION_REQUIRED`. TARGET y OTHER_CONNECTABLE son no conformes; NON_CONNECTABLE es conforme e inmutable. Q0, lectura, teardown, M4+, Certification y Pro permanecen bloqueados. El unico gate siguiente propuesto es `APPROVE_F10_10_M3_PUBLIC_DB_ACL_PRIVATE_PREFLIGHT_FREE_V2`; no autoriza remediacion, postflight ni gates reader.
