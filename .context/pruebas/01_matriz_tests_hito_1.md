...
| `T-H1-CA1-008` | `CA1 / H1-CA1` | Controles pre-main bloquean writers productivos ambiguos | `OPERABILITY_REQUIRED` | F9.9 autorizada | Ejecutar tests estaticos de workflows y preflight | `db-sync-to-pro` report-only en push, apply manual DDL-gated, FG1/FG2/FG3 environment-bound y pause fail-closed | Local / CI | `tests/test_fase09_9_pre_main_controls.py` y `tests/test_fase10_main_boundary.py` | `IMPLEMENTED_LOCAL_PENDING_CI` |
...
- `T-H1-CA1-003A` queda `PENDING_QA`: los runs F9.9 muestran fallo cerrado y ausencia de falso verde a nivel de job/steps, pero requieren revision independiente del bundle primario.
