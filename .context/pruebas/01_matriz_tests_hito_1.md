...
| `T-H1-CA1-005` | `CA1 / H1-CA1` | Secrets existen solo en CI/backend autorizado | `SECURITY_REQUIRED` | Candidate completo | Escanear tree y revisar bindings de environment | Cero secretos en Git/browser; identidad correcta | Local / CI | Security Audit post-main run `30969158679=PASS`; credential scanning cubierto por gate | `VERIFIED_CI_SECURITY` |
...
| `T-H1-CA1-007` | `CA1 / H1-CA1` | Candidate CA1-only no modifica CA2 ni otras superficies | `REGRESSION_REQUIRED` | Adenda aprobada y baseline productivo | Comparar object IDs y diff cerrado | Cero cambios DB, frontend, leads/email, backfill y artifacts CA2 | Local / CI | Boundary post-merge F10.7: 32 objetos digest `8fafc74e415d6875315e8584eb17705e24c40777675996cde9bf4ff0ccf7ddff`; cero rutas prohibidas | `VERIFIED_POST_MAIN_BOUNDARY` |
| `T-H1-CA1-008` | `CA1 / H1-CA1` | Controles pre-main bloquean writers productivos ambiguos | `OPERABILITY_REQUIRED` | F9.9 autorizada | Ejecutar tests estaticos de workflows y preflight | `db-sync-to-pro` report-only en push, apply manual DDL-gated, FG1/FG2/FG3 environment-bound y pause fail-closed | Local / CI | PR #280, `tests/test_fase09_9_pre_main_controls.py`, `tests/test_fase10_main_boundary.py`; post-main `F10 Main Boundary` job `92189531095=PASS`; DB Sync `30969158711=CANCELLED_ZERO_STEPS` | `VERIFIED_POST_MAIN_CONTROLS` |
...
- `T-H1-CA1-008` cubre controles de repositorio y contencion DB Sync; no ejecuta canary Production, schedules, Supabase, DDL/DML ni writers remotos.
- F10.7 registra Cloudflare Pages `SUCCESS` solo como publicacion tecnica del arbol de `main`; no acredita `T-H1-CA1-006` ni observacion productiva.
