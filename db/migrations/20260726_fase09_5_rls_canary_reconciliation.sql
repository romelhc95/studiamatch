-- FASE-09.5 forward-only reconciliation of the historical release canary RLS guards.
...
ON TABLE public.institutions TO anon, authenticated;
GRANT SELECT (
    id, name, slug, url, institution_id, price_pen, price_status, mode,
    course_type, category_id, duration, start_date_text, description_long,
    syllabus, target_audience, requirements, certification, benefits,
    objectives, expected_monthly_salary, seniority_level, roi_months,
    address, region, is_active, is_verified, brochure_url, start_date,
...
GRANT SELECT (id, course_id, rating_value, user_nickname, created_at)
ON TABLE public.ratings TO anon, authenticated;
GRANT SELECT (id, course_id, content, user_nickname, created_at)
ON TABLE public.reviews TO anon, authenticated;
GRANT SELECT (institution_id, production_enabled)
ON TABLE public.institution_site_profiles TO anon, authenticated;
...
DROP POLICY IF EXISTS institutions_exclude_release_canary
ON public.institutions;
...
DROP POLICY IF EXISTS profiles_exclude_release_canary
ON public.institution_site_profiles;
...
TO anon, authenticated
USING (
    COALESCE(notes, '') <> 'DB_AS_CODE_RELEASE_CANARY'
    AND EXISTS (
        SELECT 1
        FROM public.institutions AS institution_record
        WHERE institution_record.id = institution_site_profiles.institution_id
    )
);

DROP POLICY IF EXISTS courses_exclude_release_canary
ON public.courses;
...
USING (
    ...
    AND EXISTS (
        SELECT 1
        FROM public.institutions AS institution_record
        WHERE institution_record.id = courses.institution_id
    )
);
