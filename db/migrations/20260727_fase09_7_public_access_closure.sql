-- F9.7 public access closure for lead and email PII.
-- Forward-only schema, RLS, ACL, policy, and verifier changes only.
...
DROP POLICY IF EXISTS email_log_select_public ON public.email_log;
...
REVOKE SELECT (
    ...
    area_interest, budget, modality, description,
    is_late_enrollment_request, status, created_at, lead_source_type
) ON TABLE public.leads FROM PUBLIC, anon, authenticated;
GRANT INSERT (
    first_name,
    last_name,
    email,
...
REVOKE SELECT (
    ...
    id, lead_id, recipient_type, recipient_email, subject, status, resend_id,
    error_message, created_at
) ON TABLE public.email_log FROM PUBLIC, anon, authenticated;
...
DECLARE
    ...
    expected_insert_columns constant text[] := ARRAY[
        'first_name', 'last_name', 'email', 'whatsapp', 'source_page', 'type',
        'course_id', 'area_interest', 'budget', 'modality', 'description',
        'is_late_enrollment_request'
    ];
BEGIN
    ...
    IF (
        ...
        WHERE attribute.attrelid = 'public.leads'::regclass
          ...
          AND attribute.attname = ANY(expected_insert_columns)
    ) <> pg_catalog.cardinality(expected_insert_columns) THEN
        RETURN false;
    END IF;
    ...
    WHERE attribute.attrelid = 'public.leads'::regclass
      AND attribute.attname = 'lead_source_type'
      AND attribute.attnum > 0
      AND NOT attribute.attisdropped;

    IF NOT FOUND
       OR NOT actual_record.is_not_null
    ...
    FOR expected_record IN
        SELECT * FROM (VALUES
            ...
            relation_oid, column_name, type_oid, is_not_null,
            default_expression
        )
    LOOP
        SELECT
        ...
        IF NOT FOUND
           ...
           OR actual_record.is_not_null IS DISTINCT FROM
              expected_record.is_not_null
    ...
    FOR expected_record IN
        SELECT * FROM (VALUES
            ...
            relation_oid, constraint_name, constraint_type,
            definition_pattern
        )
    LOOP
        SELECT
        ...
        IF NOT FOUND
           ...
           OR (
               ...
               AND actual_record.referenced_relation <>
                   'public.courses'::regclass
           ) OR (
               SELECT pg_catalog.count(*)
    ...
    LOOP
        SELECT
            ...
            pg_catalog.pg_get_indexdef(index_record.indexrelid, 1, true)
                AS first_key,
            CASE WHEN index_record.indnkeyatts > 1 THEN
                pg_catalog.pg_get_indexdef(index_record.indexrelid, 2, true)
            END AS second_key,
            (index_record.indoption[0] & 1) = 1 AS first_descending,
            CASE WHEN index_record.indnkeyatts > 1 THEN
                (index_record.indoption[1] & 1) = 1
            ELSE false END AS second_descending,
    ...
    FOR expected_record IN
        ...
        FROM (VALUES
            ...
            (
                'leads', 'leads_service_role', 'PERMISSIVE',
                ARRAY['service_role']::name[], 'ALL', 'true', 'true'
            ),
            (
                'email_log', 'email_log_service_role', 'PERMISSIVE',
                ARRAY['service_role']::name[], 'ALL', 'true', 'true'
            )
        ) AS expected(
    ...
    FOR expected_record IN
        ...
        FROM (VALUES
            ...
            table_name, policy_name, policy_roles, command_name,
            using_expression
        )
    LOOP
        SELECT
        ...
        IF NOT FOUND
           OR actual_record.permissive <> 'PERMISSIVE'
           OR actual_record.roles IS DISTINCT FROM expected_record.policy_roles
           OR actual_record.cmd <> expected_record.command_name
           OR actual_record.using_expression IS DISTINCT FROM
              expected_record.using_expression
           OR actual_record.with_check IS NOT NULL THEN
            RETURN false;
        END IF;
    END LOOP;
    ...
    LOOP
        expected_columns := ARRAY[
            ...
            'start_date_text', 'description_long', 'syllabus',
        ...
        IF EXISTS (
            ...
            WHERE pg_catalog.has_table_privilege(
                role_name, denied_table.table_name,
            ...
            WHERE attribute.attrelid = 'public.leads'::regclass
              ...
              AND (
                  ...
                  OR pg_catalog.has_column_privilege(
                      ...
                      attribute.attname = ANY(expected_insert_columns)
                  )
              )
        ) THEN
            RETURN false;
        ...
        IF EXISTS (
            SELECT 1
            FROM pg_catalog.pg_attribute AS attribute
            WHERE attribute.attrelid = 'public.email_log'::regclass
              AND attribute.attnum > 0
              AND NOT attribute.attisdropped
              AND (
                  pg_catalog.has_column_privilege(
    ...
    IF EXISTS (
        ...
        ]::text[]) AS reader_table(table_name)
        WHERE NOT pg_catalog.has_table_privilege(
            'service_role', reader_table.table_name, 'SELECT'
        )
    ) OR EXISTS (
        SELECT 1
        FROM pg_catalog.unnest(ARRAY[
    ...
    IF NOT FOUND
       ...
       OR function_record.definition NOT LIKE
          '%jsonb_array_length(p_enriched_data) <> 1%'
       OR function_record.definition NOT LIKE
          '%Invalid atomic enrichment payload identity%' THEN
        RETURN false;
    END IF;
    ...
    FOR expected_record IN
        ...
        FROM (VALUES
            ...
            function_signature, language_name, is_security_definer,
            volatility_kind
        )
    LOOP
        SELECT
            procedure_record.oid,
            procedure_record.proowner AS owner_oid,
            owner.rolname AS owner_name,
            language_record.lanname AS language_name,
            procedure_record.prosecdef,
            procedure_record.provolatile,
            ...
            procedure_record.prosrc AS source_body,
            pg_catalog.pg_get_functiondef(procedure_record.oid) AS definition
        INTO function_record
        FROM pg_catalog.pg_proc AS procedure_record
        JOIN pg_catalog.pg_roles AS owner
        ...
        JOIN pg_catalog.pg_language AS language_record
          ON language_record.oid = procedure_record.prolang
        WHERE procedure_record.oid =
            pg_catalog.to_regprocedure(expected_record.function_signature);

        IF NOT FOUND
           OR function_record.owner_name <> 'postgres'
           OR function_record.language_name <> expected_record.language_name
           OR function_record.prosecdef IS DISTINCT FROM
              expected_record.is_security_definer
           OR function_record.provolatile <> expected_record.volatility_kind
           ...
           OR EXISTS (
               ...
               FROM pg_catalog.aclexplode(
                   COALESCE(
                       ...
                       pg_catalog.acldefault('f', function_record.owner_oid)
                   )
               ) AS acl
        ...
        IF expected_record.function_signature =
           'public.verify_fase08_hito1_contract()'
           AND (
               function_record.source_body NOT LIKE
                   '%SELECT public.verify_fase09_7_public_access_closure();%'
               OR function_record.source_body LIKE
...
ALTER FUNCTION public.verify_fase09_7_public_access_closure()
OWNER TO postgres;
REVOKE ALL ON FUNCTION public.verify_fase09_7_public_access_closure()
FROM PUBLIC, anon, authenticated, service_role CASCADE;
GRANT EXECUTE ON FUNCTION public.verify_fase09_7_public_access_closure()
TO service_role;
