...
class CleansingWorker:
    ...
    def get_pending_staging(self, limit=100):
        ...
        return self.db.select('staging_raw', filters="status=eq.pending", limit=limit)
    ...
    def process_record(self, raw):
        r_id = raw['id']
        # Use effective_url (normalized final URL) for de-duplication if available
        url = raw.get('effective_url') or raw['url']
        ...
        for loc in locations:
            ...
            res = self.db.upsert('cleansed_programs', cleansed_data, on_conflict="url")

            if res:
                created_count += 1
            else:
                logger.error(f"Error saving cleansed record for {url}")
    ...
    def update_staging_status(self, r_id, status, discard_reason=None, error_msg=None):
        ...
        self.db.patch('staging_raw', filters=f"id=eq.{r_id}", data=payload)
