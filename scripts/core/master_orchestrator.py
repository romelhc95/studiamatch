...
def get_institutions(limit=10):
    ...
    url = f"{SUPABASE_URL}/rest/v1/institutions?select=id,name,slug,website_url&limit={limit}"
    headers = {
        "apikey": SUPABASE_KEY,
...
def main():
    ...
    if not SUPABASE_URL or not SUPABASE_KEY:
        logger.error("❌ SUPABASE credentials missing in orchestrator environment.")
        sys.exit(1)
    ...
    for inst in institutions:
        ...
        inst_json = subprocess.check_output([sys.executable, "-c", f"import json; print(json.dumps({inst}))"])
        run_script("scripts/core/universal_harvester.py", [inst_json.decode().strip()])
