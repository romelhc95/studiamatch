...
HITO_CONFIG = {
    1: {
        "allowed_staged_exact": {
            ...
            ".context/backlog_tareas/tarea_001_hito_1_orquestacion_fg2_fg3_schema_base_y_seguridad.md",
    ...
    for path in sorted(git_lines(["diff", "--cached", "--name-only"])):
        ...
        if path == f".context/evidencias/hito_{hito}_informe_cumplimiento.md":
            ...
            qa_link_row = re.compile(
                rf"^\|\s*QA Gate obligatorio\s*\|\s*(?:GO|NO-GO|Pendiente)\s*\|\s*`?\.context/evidencias/hito_{hito}_qa_gate_report_\d{{8}}(?:_\d{{6}})?\.md`?\s*\|\s*$",
                re.IGNORECASE,
            )
