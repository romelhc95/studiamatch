...
try:
    from scripts.security.validate_work_package import compute_digest
except ModuleNotFoundError:
    from validate_work_package import compute_digest  # type: ignore
...
REQUIRED_INDEX_LINKS = (
    ...
    "work_packages/WP-GOV-CI-010.json",
    "work_packages/WP-GOV-CI-011.json",
    "work_packages/WP-GOV-CI-012.json",
    ...
    "backlog_tareas/governance/TASK-GOV-CI-010.md",
    "backlog_tareas/governance/TASK-GOV-CI-011.md",
    "backlog_tareas/governance/TASK-GOV-CI-012.md",
    ...
    "decisiones/ADR-0039_attestation_section_aware_fail_closed.md",
    "decisiones/ADR-0040_promocion_transaccional_y_cierre_o3.md",
    "decisiones/ADR-0041_promociones_evidencia_causal_o2_o5.md",
)
TRACKER_SECTIONS = (
    "## Verificacion",
...
GOV_CI10_BASE_TREE = "e0029083e24016b97fc8896be3be2d4285414117"
GOV_CI11_BASE_COMMIT = "cbdfe9dab373a2b427df4864b14427f3b2358789"
GOV_CI11_BASE_TREE = "99c1cda4f0091aaee35752caec69745051c41a3a"
GOV_CI12_BASE_COMMIT = "34fb06d552ea4d052bd3f942948a192a7e44a5eb"
GOV_CI12_BASE_TREE = "d53e81717a4c44d59177591c3ce69392072ec716"
...
def validate(root: Path = ROOT) -> list[str]:
    ...
    gov_ci10_task = read(root, "backlog_tareas/governance/TASK-GOV-CI-010.md")
    gov_ci11_task = read(root, "backlog_tareas/governance/TASK-GOV-CI-011.md")
    gov_ci12_task = read(root, "backlog_tareas/governance/TASK-GOV-CI-012.md")
    ...
    gov_ci10_wp = json.loads((root / ".context" / "work_packages" / "WP-GOV-CI-010.json").read_text(encoding="utf-8"))
    gov_ci11_wp = json.loads((root / ".context" / "work_packages" / "WP-GOV-CI-011.json").read_text(encoding="utf-8"))
    gov_ci12_wp = json.loads((root / ".context" / "work_packages" / "WP-GOV-CI-012.json").read_text(encoding="utf-8"))
    ...
    if "PREPARE_WP_GOV_CI_012_REQ7_R2_APPROVAL" not in state or "PREPARE_WP_GOV_CI_012_REQ7_R2_APPROVAL" not in plan or "PREPARE_WP_GOV_CI_012_REQ7_R2_APPROVAL" not in tracker:
        errors.append("NEXT_GATE_MISMATCH:GOV CI12 R2 approval gate must be next")
    ...
    if "WP-GOV-CI-010" not in state or "WP-GOV-CI-010" not in plan or "TASK-GOV-CI-010" not in gov_ci10_task:
        errors.append("GOV_CI10_WP_INVALID:missing from canonical authority")
    if "WP-GOV-CI-011" not in state or "WP-GOV-CI-011" not in plan or "TASK-GOV-CI-011" not in gov_ci11_task:
        errors.append("GOV_CI11_WP_INVALID:missing from canonical authority")
    if "WP-GOV-CI-012" not in state or "WP-GOV-CI-012" not in plan or "TASK-GOV-CI-012" not in gov_ci12_task:
        errors.append("GOV_CI12_WP_INVALID:missing from canonical authority")
    ...
    if "COMPLETE_WP_GOV_CI_011_R1_LOCAL_VALIDATION" in state + plan + tracker + read(root, "operaciones/context_graph_semantico.md"):
        errors.append("NEXT_GATE_MISMATCH:stale GOV CI11 R1 gate")
    if "PREPARE_WP_GOV_CI_012_REQ7_R2_APPROVAL" not in state + plan + tracker + read(root, "operaciones/context_graph_semantico.md"):
        errors.append("NEXT_GATE_MISMATCH:GOV CI12 R2 approval gate missing")
    if "WP-GOV-CI-010=PROPOSED_R1_LOCAL" in state + plan + tracker or "pendiente de GOV-CI10" in state + plan + tracker:
        errors.append("GOV_CI10_PUBLICATION_DRIFT:stale candidate status")
    if "GOV_CI11_PROMOTION_ENVELOPE_PENDING_R1" in state + plan + tracker or "WP-GOV-CI-011=PROPOSED_R1_LOCAL" in state + plan + tracker:
        errors.append("GOV_CI11_PUBLICATION_DRIFT:stale R1 candidate status")
    if "debe publicar parsing section-aware" in state + plan + tracker or "requiere `WP-GOV-CI-010`" in state + plan + tracker:
        errors.append("GOV_CI10_PUBLICATION_DRIFT:stale publication requirement")
    ...
    if "Apruebo WP-GOV-CI-012 de TASK-GOV-CI-012 segun manifest sha256:<D_CI12>" not in prompt:
        errors.append("NEXT_GATE_MISMATCH:GOV CI12 approval prompt missing digest placeholder")
    ...
    if f"Base: desarrollo@{GOV_CI12_BASE_COMMIT}, tree {GOV_CI12_BASE_TREE}" not in prompt:
        errors.append("NEXT_GATE_MISMATCH:GOV CI12 prompt baseline invalid")
    ...
    if gov_ci10_wp.get("id") != "WP-GOV-CI-010" or gov_ci10_wp.get("task_id") != "TASK-GOV-CI-010" or gov_ci10_wp.get("status") != "PROPOSED":
        errors.append("GOV_CI10_WP_INVALID:identity/status")
    if gov_ci10_wp.get("target_level") != "R2":
        errors.append("GOV_CI10_WP_INVALID:target")
    if gov_ci10_wp.get("baseline", {}).get("candidate_commit") != GOV_CI10_BASE_COMMIT or gov_ci10_wp.get("baseline", {}).get("candidate_tree") != GOV_CI10_BASE_TREE:
        errors.append("GOV_CI10_WP_INVALID:baseline")
    failure10 = gov_ci10_wp.get("post_merge_attestation_duplicate_failure", {})
    if failure10.get("failed_pr") != 441 or failure10.get("failed_run") != 32666126533 or failure10.get("primary_failure") != "POST_MERGE_ATTESTATION_DUPLICATE":
        errors.append("GOV_CI10_FAILURE_INVALID")
    section_contract = gov_ci10_wp.get("section_aware_attestation_contract", {})
    if section_contract.get("no_body_fallback") is not True or section_contract.get("ordinary_desarrollo_result") != "NOT_APPLICABLE":
        errors.append("GOV_CI10_SECTION_CONTRACT_INVALID")
    bootstrap10 = gov_ci10_wp.get("promotion_request_bootstrap", {})
    if bootstrap10.get("static_request_status") != "REQUESTED_JIT_SINGLE_USE" or bootstrap10.get("final_wp") != "WP-GOV-CI-010" or len(bootstrap10.get("grant_request_ids", [])) != 4:
        errors.append("GOV_CI10_BOOTSTRAP_INVALID")
    if "CANDIDATE_R1_LOCAL" not in gov_ci10_task:
        errors.append("GOV_CI10_TASK_INVALID:status")
    if gov_ci11_wp.get("id") != "WP-GOV-CI-011" or gov_ci11_wp.get("task_id") != "TASK-GOV-CI-011" or gov_ci11_wp.get("status") != "PROPOSED":
        errors.append("GOV_CI11_WP_INVALID:identity/status")
    if gov_ci11_wp.get("target_level") != "R2" or gov_ci11_wp.get("candidate_digest") != compute_digest(gov_ci11_wp):
        errors.append("GOV_CI11_WP_INVALID:target/digest")
    if gov_ci11_wp.get("baseline", {}).get("candidate_commit") != GOV_CI11_BASE_COMMIT or gov_ci11_wp.get("baseline", {}).get("candidate_tree") != GOV_CI11_BASE_TREE:
        errors.append("GOV_CI11_WP_INVALID:baseline")
    if gov_ci11_wp.get("promotion_jit_envelope", {}).get("schema") != "promotion-jit-envelope-v1" or gov_ci11_wp.get("promotion_environment_desired_state", {}).get("can_admins_bypass") is not False:
        errors.append("GOV_CI11_ENVELOPE_INVALID")
    if gov_ci11_wp.get("o3_closure", {}).get("cloudflare_pages_app_id") != 85455 or gov_ci11_wp.get("o3_closure", {}).get("db_sync_expected_result") != "NO_DB_CHANGES":
        errors.append("GOV_CI11_O3_INVALID")
    if "CANDIDATE_R1_LOCAL" not in gov_ci11_task:
        errors.append("GOV_CI11_TASK_INVALID:status")
    if gov_ci12_wp.get("id") != "WP-GOV-CI-012" or gov_ci12_wp.get("task_id") != "TASK-GOV-CI-012" or gov_ci12_wp.get("status") != "PROPOSED":
        errors.append("GOV_CI12_WP_INVALID:identity/status")
    if gov_ci12_wp.get("target_level") != "R2" or gov_ci12_wp.get("candidate_digest") != compute_digest(gov_ci12_wp):
    ...
    if "PR #441" not in state + tracker + plan or GOV_CI10_BASE_COMMIT not in state + tracker + plan or GOV_CI10_BASE_TREE not in state + tracker + plan or "32666126533" not in state + tracker + plan or "POST_MERGE_ATTESTATION_DUPLICATE" not in state + tracker + plan:
    ...
    if "PR #444" not in state + tracker + plan or "PR #446" not in state + tracker + plan or "PR #448" not in state + tracker + plan or GOV_CI12_BASE_COMMIT not in state + tracker + plan or GOV_CI12_BASE_TREE not in state + tracker + plan:
        errors.append("GOV_CI11_R2_HISTORY_MISSING:PR444_PR446_PR448")
