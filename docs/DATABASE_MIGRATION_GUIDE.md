# Guía de Migración de Base de Datos: Categorías Dinámicas

Esta guía proporciona los pasos detallados para ejecutar manualmente el script de migración para el nuevo sistema de categorías dinámicas en el dashboard de Supabase. Esto resuelve el 'Bloqueo SQL' (Tarea 4, Fase 15).

## Pasos para la Ejecución Manual

1. Inicia sesión en el dashboard de [Supabase](https://app.supabase.com/).
2. Selecciona el proyecto **StudIAMatch**.
3. En el menú lateral izquierdo, ve a la sección **SQL Editor**.
4. Haz clic en **New query** para crear una nueva pestaña de consulta.
5. Copia el script SQL consolidado que se muestra a continuación.
6. Pega el script en el editor de Supabase.
7. Haz clic en el botón **Run** (o presiona `Cmd+Enter` / `Ctrl+Enter`) para ejecutar la migración.
8. Verifica que el panel de resultados muestre un mensaje de éxito sin errores.
9. Puedes validar la creación de tablas en la sección **Table Editor** revisando la existencia de `categories` y `category_rules`.

## Script SQL Consolidado

A continuación se presenta el script completo extraído de `db/migrations/20260412_dynamic_categories.sql`:
