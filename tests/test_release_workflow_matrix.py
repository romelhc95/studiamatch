...
def test_gov_ci9_target_aware_promotions_are_documented_and_gated() -> None:
    ...
    for branch in (
        "promote/gov-hom-009-o2-req1",
        "promote/gov-hom-009-o3-req1",
        "promote/gov-hom-009-o4-req1",
        "promote/gov-hom-009-o5-req1",
    ):
    ...
    assert "owner-only-protected-branch-updates" in source(".context/work_packages/WP-GOV-CI-009.json")
...
def test_o5_uses_promotion_boundary_and_skips_governance_preflight() -> None:
    ...
    assert "github.event.pull_request.head.ref == 'promote/gov-hom-009-o5-req1'" in governance_job
    assert "desarrollo:promote/gov-hom-009-o5-req1" in aggregate
...
def test_o2_o3_o4_promotion_pairs_remain_gated() -> None:
    workflow = source(".github/workflows/security-audit.yml")
    for pair in (
        "certificacion:promote/gov-hom-009-o2-req1",
        "main:promote/gov-hom-009-o3-req1",
        "certificacion:promote/gov-hom-009-o4-req1",
    ):
...
def test_certification_and_main_require_exact_promotion_route() -> None:
    ...
    assert "protected branch PR requires exact HOM-009 promotion route" in path_boundary
    assert '[ "$base_ref" = "certificacion" ] || [ "$base_ref" = "main" ]' in path_boundary


def test_gov_ci9_owner_only_desired_state_is_documented() -> None:
    manifest = source(".context/work_packages/WP-GOV-CI-009.json")
