...
test('Course detail page responsiveness', async ({ page }) => {
  ...
  // Lead capture is disabled by default unless the exact build flag is "true".
  await expect(page.locator('[data-lead-capture-state="disabled"]').first()).toBeVisible();
  await expect(page.locator('[data-lead-capture-form="course-detail"]')).toHaveCount(0);
  await expect(page.locator('[data-pii-control]')).toHaveCount(0);
});
