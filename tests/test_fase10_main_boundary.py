...
def test_f10_main_boundary_gate_is_present_in_security_audit() -> None:
    ...
    assert "F9.9 Pre-Main Repository Controls" in workflow
    assert "tests/test_fase09_9_pre_main_controls.py tests/test_fase10_main_boundary.py" in workflow
    assert "needs.fase09-9-pre-main-controls.result" in workflow
    assert "F99_PRE_MAIN: ${{ needs.fase09-9-pre-main-controls.result }}" in workflow
...
def test_pre_main_controls_do_not_touch_denied_runtime_surfaces() -> None:
    ...
    assert "Paths permitidos para ese paquete posterior" in plan
