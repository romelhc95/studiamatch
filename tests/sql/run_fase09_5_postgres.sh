...
MANIFEST="$ROOT/db/manifests/fase09_5_rls_candidate.json"
...
SUCCESSOR="$ROOT/db/migrations/20260726_fase09_5_rls_canary_reconciliation.sql"
...
python3 - "$MANIFEST" "$package_wrapper" "$probe_wrapper" \
  "$prefix3_wrapper" "$prefix4_wrapper" <<'PY'
import sys
from pathlib import Path
...
for prefix_size, wrapper_path in zip((3, 4), sys.argv[4:]):
    expected_prefix = {
...
WHERE schemaname = 'public'
  AND tablename IN ('institutions', 'institution_site_profiles', 'courses');
SQL
}

for prefix_size in 3 4; do
  prefix_wrapper="$prefix3_wrapper"
  [[ "$prefix_size" -eq 3 ]] || prefix_wrapper="$prefix4_wrapper"
  reset_database
  ...
  prefix_state="$({
    psql -X --quiet --tuples-only --no-align --set=ON_ERROR_STOP=1 \
      ...
      "SET ROLE service_role; SELECT (SELECT pg_catalog.count(*) FROM public.supabase_migrations) = 5 AND public.verify_fase08_hito1_contract() AND public.verify_fase09_5_rls_canary_reconciliation();"
  } | tail -n 1 | tr -d '[:space:]')"
  [[ "$prefix_state" == "t" ]]
done
...
replay_verifiers="$({
  psql -X --quiet --tuples-only --no-align --set=ON_ERROR_STOP=1 \
    "$TEST_DATABASE_URL" --command \
    'SET ROLE service_role; SELECT public.verify_fase08_hito1_contract() AND public.verify_fase09_5_rls_canary_reconciliation();'
} | tail -n 1 | tr -d '[:space:]')"
[[ "$replay_verifiers" == "t" ]]

plan_manifest 0

# A final-verifier failure rolls back migrations and all five ledger markers.
reset_database
psql -X --quiet --set=ON_ERROR_STOP=1 "$TEST_DATABASE_URL" \
  --file "$BASELINE" >/dev/null
psql -X --quiet --set=ON_ERROR_STOP=1 "$TEST_DATABASE_URL" \
...
rollback_state="$({
  psql -X --quiet --tuples-only --no-align --set=ON_ERROR_STOP=1 \
    "$TEST_DATABASE_URL" --command \
    "SELECT (SELECT count(*) FROM public.supabase_migrations) = 0 AND pg_catalog.to_regprocedure('public.verify_fase09_5_rls_canary_reconciliation()') IS NULL AND EXISTS (SELECT 1 FROM pg_catalog.pg_policies WHERE schemaname = 'public' AND policyname = 'fase09_5_atomic_fault');"
} | tr -d '[:space:]')"
[[ "$rollback_state" == "t" ]]
